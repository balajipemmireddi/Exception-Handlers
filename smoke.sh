...
# =================================================================================
# SMOKE TEST SCRIPT - HOTEL-BALL (FINAL VERSION)
# =================================================================================
...
check_endpoint "Public - Search Hotels" "/api/hotels/search?location=Mumbai" 200
...
if [ -n "$TOKEN" ]; then
    ...
    check_endpoint "Protected - Admin Only (Should be 403)" "/api/test/admin" 403 "$TOKEN"
else
    echo -e "${COLOR_FAILURE}ERROR: Skipping protected endpoints because token was not retrieved.${COLOR_RESET}" >&2
fi

# 4. Error Case
check_endpoint "Error - Login Bad Request" "/api/auth/login" 400
